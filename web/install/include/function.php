		...
		if(isset($lstr[0]) && $lstr[0]!='#')
		{
			...
			switch($prefix)
			{
				...
				default :
				{
					...
					{
						$rcount   = 1;
						$sqlStr   = str_ireplace(array('we_','we_'),$db_pre,join($sqlArray),$rcount); //更换表前缀
						$sqlArray = array();
...
function install_sql()
{
	...
	//检测SQL安装文件
	$sql_file = ROOT_PATH.'./install/wuzu.sql';
	...
	//检测测试数据SQL文件
	$sql_test_file = ROOT_PATH.'./install/wuzu.sql';
	...
	//插入管理员数据
	$adminSql = 'insert into we_user(`u_name`,`u_pwd`) values ("'.$admin_user.'","'.md5($admin_pwd).'")';
